      ...
      <div>
        {CERTIFICATIONS.map((certification, index) => (
          <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
            ...
            >
              <p className="mb-2 text-sm text-neutral-400">{certification.yearMonth}</p>
            </motion.div>
            <motion.div 
              whileInView={{ opacity: 1, x: 0 }}
              initial={{ opacity: 0, x: 100 }}
            ...
            >
              <h6 className="mb-2 font-semibold">
                {certification.title} - <span className="text-sm text-purple-100">{certification.organization}</span>
              </h6>
              <p className="mb-4 text-neutral-400">{certification.description}</p>
              {/* Displaying technologies as tags */}
